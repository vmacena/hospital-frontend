...
import { FaCalendarAlt, FaUserMd, FaUser, FaFileAlt, FaSignOutAlt, FaChartBar } from 'react-icons/fa';
...
export default function PatientDashboard() {
  ...
  const [activeView, setActiveView] = useState<'viewAppointments' | 'bookAppointment' | 'viewExams' | 'profile' | 'stats'>('viewAppointments');
  ...
  useEffect(() => {
    const fetchAppointmentsAndExams = async () => {
      try {
        ...
        const profileResponse = await fetch('http://localhost:8080/patient/:id', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
  ...
  const handleViewChange = (view: 'viewAppointments' | 'bookAppointment' | 'viewExams' | 'profile' | 'stats') => {
    setIsAnimating(true);
    setTimeout(() => {
      setActiveView(view);
      ...
      <div className={styles.panel}>
        ...
        <div className={styles.tablesContainer}>
          {activeView === 'viewAppointments' && (
            <div className={`${styles.tableWrapper} ${isAnimating ? styles.fadeOut : styles.fadeIn}`}>
          ...
          {activeView === 'stats' && stats && (
            <div className={`${styles.tableWrapper} ${isAnimating ? styles.fadeOut : styles.fadeIn}`}>
              <h2>Estatísticas</h2>
              <p><strong>Total de Consultas:</strong> {stats.totalAppointments}</p>
              <p><strong>Total de Exames:</strong> {stats.totalExams}</p>
              <p><strong>Próximas Consultas:</strong> {stats.upcomingAppointments}</p>
              <p><strong>Exames Concluídos:</strong> {stats.completedExams}</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className={styles.sidebar}>
      ...
      <ul>
        ...
        <li>
          <button onClick={() => setActiveView('bookAppointment')} className={styles.sidebarButton}>
            <FaUserMd style={{ marginRight: '8px' }} />
            Marcar Consulta
          </button>
        </li>
        <li>
          <button onClick={() => setActiveView('viewExams')} className={styles.sidebarButton}>
            <FaFileAlt style={{ marginRight: '8px' }} />
            Ver Exames
          </button>
        </li>
        <li>
          <button onClick={() => setActiveView('profile')} className={styles.sidebarButton}>
            <FaUser style={{ marginRight: '8px' }} />
            Perfil
          </button>
        </li>
        <li>
          <button onClick={() => setActiveView('stats')} className={styles.sidebarButton}>
